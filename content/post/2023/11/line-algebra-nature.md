...
\\(A^{-1}A\\) ，\\(A^{-1}\\) 是逆变换，inverse transformation。就是说结果是\\[ \left[\begin{matrix}
1 & 0 \\\\
0 & 1
\end{matrix}\right] \\] ，现在的理解就很清楚了。
...
\\[
{\hat{i}}(v\_{2}w\_{3}-v\_{3}w\_{2})+{\hat{j}}(v\_{3}w\_{1}-v\_{1}w\_{3})+{\hat{k}}(v\_{1}w\_{2}-v\_{2}w\_{1})
\\]
