...
sudo apt install cmake libtool-bin pass isync gtags
...
这个问题换了一下，使用 melpa-orig 这个手动重新安装这个包解决了一个小报错。
...
emacs 包使用的 emacs-rime 也需要下载：sudo apt install librime-dev
...
#+end_src
这个非常地好。我的五笔反查的配置到底是哪里做的？不记得了，不懂原理的配置不算会，只能说是瞎蒙的。
