...
\extrainfo{简单介绍一下概念和学习路线}
\emergencystretch 3em
\author{郭隆基}
\date{\textit{[2023-09-25 一 10:31]}}
\title{tty和x的切换流程}
\hypersetup{
 pdfauthor={郭隆基},
 pdftitle={tty和x的切换流程},
...
这篇文档可以在：\url{https://guolongji.xyz/post/tty-x-switch/} 这里看到更新。

\chapter{概念}
\label{sec:orgc9711e3}
...
VT 驱动的代码在 drivers/tty/vt/vt.c 当中。其中的 visual\_init 函数中调用 con\_init 函数。

流程是内核启动初始化。

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{tty-x-switch.org.assets/img_20230926_090023.png}
\caption{\emph{tty 子系统架构图}}
\end{figure}


\chapter{如何将一个 tty 和一个 xorg 关连起来？}
\label{sec:org0505a5c}

分配新用户的 TTY 涉及到内核中几个关键的函数。以下是其中的一些函数：

1.get\_unused\_tty\_index()：用于获取一个未被使用的 TTY 索引。每个 TTY 都有一个唯一的索引号，内核使用这个索引号
来标识不同的 TTY 设备。

2.tty\_alloc\_driver()：用于为新的 TTY 实例分配一个 TTY 驱动结构体。TTY 驱动结构体保存了与 TTY 设备相关的信息和操作。

3.tty\_init\_dev()：用于初始化新的 TTY 设备。这个函数会设置 TTY 的状态、配置串口传输参数以及注册 TTY 设备到系统中。

4.tty\_open()：用于打开 TTY 设备。当一个用户切换到一个新的 TTY 时，内核会调用这个函数来打开对应的 TTY 设备。

5.vt\_do\_activate()：用于激活虚拟终端（Virtual Terminal，VT）。在 Linux 中，每个 TTY 都对应一个 VT，用于提供文
...
\chapter{dbus}
\label{sec:org3e88cdf}

\begin{itemize}
\item lightdm(org.freedesktop.DisplayManager)
\end{itemize}
...
\begin{itemize}
\item greeter(lightdm-deepin-greeter)

\item deepin-authenticate(com.deepin.daemon.Authenticate)

\item systemd-logind(org.freedesktop.login1)

\item dde-lockservice(org.deepin.dde.LockService1)

\item dde-system-daemon(org.deepin.dde.Accounts1)

\item accounts-daemon(org.freedesktop.Accounts)
\end{itemize}
...
\chapter{linux 图形学中的一些概念}
\label{sec:orgbd7870d}

\section{DM，WM 与 X Display Manager Control Protocol（XDMCP）}
\label{sec:org1744fec}
...
、awesome、i3wm、mutter（gnome）\ldots{}\ldots{}
...
lightdm 的配置文件在： \texttt{/etc/lightdm/} 里；

xorg 的配置文件在这里： \texttt{/etc/X11/} 里；
...
\chapter{AMD 显卡的架构演进}
\label{sec:orgd2b21a8}

\url{https://medium.com/high-tech-accessible/an-overview-of-amds-gpu-architectures-884432a717a6}

\begin{center}
\includegraphics[width=.9\linewidth]{tty-x-switch.org.assets/img_20231101_111732.jpg}
\end{center}
...
\begin{center}
\includegraphics[width=.9\linewidth]{tty-x-switch.org.assets/img_20231101_111814.jpg}
\end{center}
...
\begin{center}
\includegraphics[width=.9\linewidth]{tty-x-switch.org.assets/img_20231101_111850.jpg}
\end{center}
...
\chapter{AMD 驱动代码的模块划分和代码介绍}
\label{sec:orge5e82a0}
...
x86-kernel/drivers/gpu/drm/amd/amdgpu/amdgpu\_atombios.c

x86-kernel/drivers/gpu/drm/amd/amdgpu/amdgpu\_i2c.c

x86-kernel/drivers/gpu/drm/amd/display/amdgpu\_dm/amdgpu\_dm.c
...
oland 显卡用的是 dce\_v6\_0 这个模块：



\section{激活 tty 的流程}
\label{sec:orgc7a79f7}

分配新用户的 TTY 涉及到内核中几个关键的函数。以下是其中的一些函数：

1.get\_unused\_tty\_index()：用于获取一个未被使用的 TTY 索引。每个 TTY 都有一个唯一的索引号，
内核使用这个索引号来标识不同的 TTY 设备。

2.tty\_alloc\_driver()：用于为新的 TTY 实例分配一个 TTY 驱动结构体。TTY 驱动结构体保存了与
TTY 设备相关的信息和操作。

3.tty\_init\_dev()：用于初始化新的 TTY 设备。这个函数会设置 TTY 的状态、配置串口传输参数以及注
册 TTY 设备到系统中。

4.tty\_open()：用于打开 TTY 设备。当一个用户切换到一个新的 TTY 时，内核会调用这个函数来打开对应的 TTY 设备。

5.vt\_do\_activate()：用于激活虚拟终端（Virtual Terminal，VT）。在 Linux 中，每个 TTY 都对应一个
...
ioctl(fd, VT\_ACTIVATE, vt)，通过这个调用内核。

对应的是内核的 drivers/tty/vt/vt\_ioctl.c 里面的两个处理函数：

vt\_ioctl 和 vt\_compat\_ioctl 两个处理函数：
...
drm\_mode\_getconnector

drm\_helper\_probe\_single\_connector\_modes

amdgpu\_connector\_dvi\_detect

amdgpu\_connector\_get\_edid

drm\_get\_edid

drm\_do\_get\_edid

drm\_do\_probe\_ddc\_edid

bit\_xfer

drivers/gpu/drm/amd/amdgpu/amdgpu\_connectors.c
...
我对 edid 不太了解， \textbf{现在怀疑是 VGA 的循环读 edid 和切换用户时读 edid 同时进行的情况下在 i2c 的层面有了干扰}

\chapter{用户与系统 xorg 进程对应方法}
\label{sec:orga19391c}
...
\chapter{使用 perf 来调试}
\label{sec:orgb6b2420}

关于 perf 的使用可以看另一篇博客： \url{https://guolongji.xyz/post/use-perf-and-flame-graph/}
...
\begin{center}
\includegraphics[width=.9\linewidth]{tty-x-switch.org.assets/img_20231026_172216.jpg}
\end{center}

xorg 的时钟占比为 94\%，而 drm\_get\_edid 的时钟占比高达 32\%。

显然被测试的 oland 显卡 drm\_do\_probe\_ddc\_edid 这个过程是有时候比较慢的。一直切换会由“经常不黑屏”的状态变为经常
黑屏的状态。所以猜测是存在什么循环。


\chapter{为什么 xorg 的数量比登陆的用户数多一个？}
\label{sec:orge9ee6e3}
...
\chapter{能不能写一个切换用户的脚本呢？}
\label{sec:org2e59c8b}
...
\url{https://blog.51cto.com/u\_15155099/2767298}


HDMI 拔出正常逻辑应该是：HPD 探测到电压变化触发中断，接下来 DDC 读取显示器 EDID 返回失败，最终到 dvi\_detect 函数
...
\chapter{什么是 EDID？}
\label{sec:orgad54044}

edid 核心的内容都在这里了：

\url{https://www.graniteriverlabs.com.cn/technical-blog/edid-overview/}

\url{https://en.wikipedia.org/wiki/Extended\_Display\_Identification\_Data}

\url{https://www.wpgdadatong.com.cn/blog/detail/72670}


\begin{center}
\includegraphics[width=.9\linewidth]{tty-x-switch.org.assets/img_20231027_145247.jpg}
\end{center}
...
\begin{center}
\includegraphics[width=.9\linewidth]{tty-x-switch.org.assets/img_20231027_152110.jpg}
\end{center}
...
\url{https://winddoing.github.io/post/47714.html}

有一个站可以解析 edid： \url{http://www.edidreader.com/}

开源的解析工具很多，\url{https://github.com/dgallegos/edidreader} ，供参考。
...
\chapter{FQA}
\label{sec:org19817ee}

\begin{itemize}
\item 为什么系统下和芯片中都需要包含固件代码？（显卡 vbios 和系统下的 bin）
\end{itemize}
